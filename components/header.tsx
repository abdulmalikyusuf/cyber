...
import { subNavItems } from "@/config/home";
...
function Header() {
	...
	return (
		<header className="font-sf">
			<div className="bg-white py-6 px-4 sm:px-160 sm:py-4">
				<div className="flex justify-between items-center sm:justify-normal sm:gap-8">
					<Image
						src="/logo.svg"
						alt="Cyber Logo"
					...
					<div className="flex gap-x-8 max-sm:hidden">
						<SearchBar className="w-[433px]" />
						<ul className="flex items-center gap-x-[52px]">
							{pages.map((page) => (
								<li key={page.href}>
									<Link
										href={page.href}
										className="text-main-black/30 active:text-main-black/100 font-medium"
									>
										{page?.title}
									</Link>
								</li>
			...
			<nav className="bg-secondary-charcoal max-sm:hidden px-160 py-2">
				<div className="grid grid-cols-6 divide-x divide-main-white/20">
					{subNavItems.map((item) => (
						...
						>
							...
							/>
							<span className="text-base leading-8 font-medium text-main-white/50">
								{item.title}
							</span>
						</button>
					))}
